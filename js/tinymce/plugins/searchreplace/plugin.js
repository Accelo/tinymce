...
var searchreplace = (function () {
  ...
  var $_ew7y54isjdxlevm2 = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_ew7y54isjdxlevm2.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_cssr9fiqjdxlevlo.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_cssr9fiqjdxlevlo.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_cssr9fiqjdxlevlo.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_cssr9fiqjdxlevlo.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_cssr9fiqjdxlevlo.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_cssr9fiqjdxlevlo.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_cssr9fiqjdxlevlo.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_cssr9fiqjdxlevlo.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_cssr9fiqjdxlevlo.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_cssr9fiqjdxlevlo.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_cssr9fiqjdxlevlo.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_cssr9fiqjdxlevlo.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_cssr9fiqjdxlevlo.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_cssr9fiqjdxlevlo.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_cssr9fiqjdxlevlo.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_eujziuiujdxlevmm = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_eujziuiujdxlevmm.open(editor, currentIndexState);
    });
  };
  var $_40gdg5itjdxlevmd = { register: register };
  ...
  var $_8n2cx4ivjdxlevn1 = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_40gdg5itjdxlevmd.register(editor, currentIndexState);
    $_8n2cx4ivjdxlevn1.register(editor, currentIndexState);
    return $_9wwt2wipjdxlevli.get(editor, currentIndexState);
  });
  function Plugin () {
  }
