...
var textpattern = (function () {
  ...
  var $_4io1jzqhjdxlexyj = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_3r9an1qpjdxlexzm.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_3r9an1qpjdxlexzm.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_3r9an1qpjdxlexzm.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_cejlm7qmjdxlexyz.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_cejlm7qmjdxlexyz.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_cejlm7qmjdxlexyz.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_366ssoqljdxlexyt.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_366ssoqljdxlexyt.checkKeyCode(keyCodes, e)) {
        $_366ssoqljdxlexyt.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_366ssoqljdxlexyt.checkCharCode(charCodes, e)) {
  ...
  var $_bs52a0qijdxlexyn = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_4io1jzqhjdxlexyj.getPatterns(editor.settings));
    $_bs52a0qijdxlexyn.setup(editor, patternsState);
    return $_c4fgq2qgjdxlexyh.get(patternsState);
  });
  function Plugin () {
  }
